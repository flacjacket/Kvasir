...
//16-bit counter/timer
    namespace Ct16b0Ir{    ///<Interrupt Register (IR). The IR can be written to clear interrupts. The IR can be read to identify which of five possible interrupt sources are pending.
    ...
    namespace Ct16b0Tcr{    ///<Timer Control Register (TCR). The TCR is used to control the Timer Counter functions. The Timer Counter can be disabled or reset through the TCR.
    ...
    namespace Ct16b0Tc{    ///<Timer Counter (TC). The 16-bit TC is incremented every PR+1 cycles of PCLK. The TC is controlled through the TCR.
    ...
    namespace Ct16b0Pr{    ///<Prescale Register (PR). When the Prescale Counter (below) is equal to this value, the next clock increments the TC and clears the PC.
    ...
    namespace Ct16b0Pc{    ///<Prescale Counter (PC). The 16-bit PC is a counter which is incremented to the value stored in PR. When the value in PR is reached, the TC is incremented and the PC is cleared. The PC is observable and controllable through the bus interface.
    ...
    namespace Ct16b0Mcr{    ///<Match Control Register (MCR). The MCR is used to control if an interrupt is generated and if the TC is reset when a Match occurs.
        ...
        namespace Mr0iValC{
            ...
            constexpr Register::FieldValue<decltype(mr0i)::Type,Mr0iVal::d> d{};
        }
    ...
    namespace Ct16b0Ccr{    ///<Capture Control Register (CCR). The CCR controls which edges of the capture inputs are used to load the Capture Registers and whether or not an interrupt is generated when a capture takes place.
    ...
    namespace Ct16b0Cr0{    ///<Capture Register 0 (CR0). CR0 is loaded with the value of TC when there is an event on the CT16B0_CAP0 input.
    ...
    namespace Ct16b0Emr{    ///<External Match Register (EMR). The EMR controls the match function and the external match pins CT16B0_MAT[2:0].
    ...
    namespace Ct16b0Ctcr{    ///<Count Control Register (CTCR). The CTCR selects between Timer and Counter mode, and in Counter mode selects the signal and edge(s) for counting.
    ...
    namespace Ct16b0Pwmc{    ///<PWM Control Register (PWMCON). The PWMCON enables PWM mode for the external match pins CT16B0_MAT[2:0].
        ...
        enum class Pwmen0Val {
            ...
            pwmmode=0x00000001,     ///<PWM mode is enabled for CT16Bn_MAT0.
        };
        ...
        enum class Pwmen1Val {
            ...
            pwmmode=0x00000001,     ///<PWM mode is enabled for CT16Bn_MAT1.
        };
        ...
        enum class Pwmen2Val {
            ...
            pwmmode=0x00000001,     ///<PWM mode is enabled for match channel 2 or pin CT16B0_MAT2.
        };
        ...
        enum class Pwmen3Val {
            ...
            pwmmode=0x00000001,     ///<PWM mode is enabled for match channel 3match channel 3.
        };
        ...
        namespace Pwmen3ValC{
            ...
            constexpr Register::FieldValue<decltype(pwmen3)::Type,Pwmen3Val::pwmmode> pwmmode{};
        }
        ///Reserved, user software should not write ones to reserved bits. The value read from a reserved bit is not defined.
        constexpr Register::FieldLocation<Addr,Register::maskFromRange(31,4),Register::ReadWriteAccess,unsigned> reserved{}; 
    }
