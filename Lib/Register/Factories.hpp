...
namespace Detail
{
        ...
        : Action<FieldLocation<TAddress, Mask, Access, TFieldType>, WriteLiteralAction<Mask>>
    {
        static_assert(onlyOneBitSet(Mask),
                      "Register::set only works on single bits. Use Register::write to write "
                      "values to wider bit fields");
    };
    ...
    {
        static_assert(onlyOneBitSet(Mask), "Register::clear only works on single bits. Use "
                                           "Register::write to write values to wider bit fields");
    };

    // special case for clearing toggle bits. Writing the value back will clear these, therefore
    // using a xor of 0 will clear.
    template <typename TAddress, unsigned Mask, AccessType AT, ReadActionType RAT,
              typename TFieldType>
    struct Clear<FieldLocation<TAddress, Mask, Access<AT, RAT, ModifiedWriteValueType::oneToToggle>,
                               TFieldType>>
        : Action<FieldLocation<TAddress, Mask, Access<AT, RAT, ModifiedWriteValueType::oneToToggle>,
                               TFieldType>,
                 XorLiteralAction<0>>
    {
    };
...
template <typename T>
constexpr inline std::enable_if_t<Detail::IsFieldLocation<T>::value, Action<T, ReadAction>> read(T)
{
    return {};
}
...
template <typename T>
constexpr std::enable_if_t<Detail::IsFieldLocation<T>::value, Detail::SetT<T>> set(T)
{
    return {};
}
...
template <typename T>
constexpr std::enable_if_t<Detail::IsFieldLocation<T>::value, Detail::ClearT<T>> clear(T)
{
    return {};
}
...
template <typename T>
constexpr std::enable_if_t<Detail::IsFieldLocation<T>::value, Detail::Reset<T>> reset(T)
{
...
write(T, Detail::GetFieldTypeT<T> in)
{
    ...
    return Action<T, WriteAction>{Detail::GetMask<T>::value & (unsigned(in) << Detail::maskStartsAt(
                                                                   Detail::GetMask<T>::value))};
}
...
constexpr inline std::enable_if_t<(Detail::IsFieldLocation<T>::value && MPL::IsValue<U>::value),
                                  Detail::WriteT<T, Detail::ValueToUnsigned<U>::value>>
    write(T, U)
{
    static_assert(Detail::WriteLocationAndCompileTimeValueTypeAreSame<T, U>::value,
                  "type mismatch: the FieldLocation field type and the compile time Value type "
                  "must be the same");
    return {};
}
...
constexpr inline decltype(write(T{}, MPL::Value<typename T::DataType, V>{})) write(FieldValue<T, V>)
{
    return {};
}
...
template <typename T, typename U, typename... Ts>
constexpr brigand::list<decltype(write(std::declval<T>())), decltype(write(std::declval<U>())),
